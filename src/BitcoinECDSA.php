...
class BitcoinECDSA
{
    ...
    public function generateRandom256BitsHexaString($extra = 'FkejkzqesrfeifH3ioio9hb55sdssdsdfOO:ss')
    {
        do {
            ...
            gmp_init($res, 16),
            gmp_sub($this->n, gmp_init(1, 10))
        ) === 1); // make sure the generate string is smaller than n
    ...
    public function doublePoint(array $pt)
    {
        ...
        if ($gcd !== '1') {
            throw new \Exception(
                'This library doesn\'t yet supports point at infinity. See https://github.com/BitcoinPHP/BitcoinECDSA.php/issues/9'
            );
        }
    ...
    public function signHash($hash, $nonce = null)
    {
        ...
        $signature = '02' . dechex(strlen(hex2bin($points['R']))) . $points['R'] . '02' . dechex(
                strlen(hex2bin($points['S']))
            ) . $points['S'];
